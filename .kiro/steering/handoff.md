...
Create a structured handoff document that captures everything the next session (or agent) needs to seamlessly continue this work. This is the **Write + Compress** pattern — you're externalizing the session's memory into a persistent file AND compressing it into just the essentials. When the next session's intended focus is known, tailor the handoff toward that focus instead of producing a generic recap.
...
- Is there a known next-session focus that should shape the next steps and context?
- Which existing artifacts already capture detail that should be referenced instead of repeated?
- What sensitive data appeared in the session that must be redacted?

### 2. Gather Current State

Collect the current branch, last commit, relevant uncommitted changes, verification results, and any existing artifacts that the next session should inspect directly.

### 3. Write the Handoff Document

Save to: `HANDOFF.md` in the current working directory (or the worktree root if in a worktree).

If the current session has a known next focus, tailor `In Progress / Next Steps`, `Context for Next Session`, and `Suggested Skills` toward that focus.

Do not duplicate existing artifacts. Reference plans (for example `docs/plans/...`), PRDs, ADRs, issues, commits, diffs, and existing docs by path, URL, or hash instead of copying their contents into the handoff.

Redact secrets, credentials, tokens, PII, and similarly sensitive data. Sensitive information must not be included in `HANDOFF.md`.
...
- `path/to/new-file.py` — [NEW: what this file does]
...
- `[skill-name]` — [why it is relevant to the known next focus]
...
- Be under 100 lines (concise by reference, not repetition — link to files and artifacts rather than duplicating content)
...
- Redact secrets, credentials, tokens, PII, and similarly sensitive data

## Anti-patterns

- Don't include full file contents — reference paths instead
- Don't copy plans, PRDs, ADRs, issues, commits, diffs, or existing docs already captured elsewhere — reference them by path, URL, or hash, such as `docs/plans/...` or a commit hash
- Don't include secrets, credentials, tokens, PII, or similarly sensitive information
