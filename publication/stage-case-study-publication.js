...
const stagedStoreOutput = (store) => `// Generated by tools/stage-case-study-publication.js.\nexport const stagedCaseStudyPublication = ${JSON.stringify(store, null, 2)};\n`;

export const readStagedCaseStudyPublication = (stagedPath) => {
  ...
  if (!Array.isArray(existing.records) || !existing.claims || typeof existing.claims !== 'object' || Array.isArray(existing.claims)) throw new Error('Staged publication store must contain records and claims');
  if (existing.assets !== undefined && (!existing.assets || typeof existing.assets !== 'object' || Array.isArray(existing.assets))) throw new Error('Staged publication store assets must be an object');
  return existing;
};

export const writeStagedCaseStudyPublication = (stagedPath, store) => {
  ...
  try { writeFileSync(temporaryPath, stagedStoreOutput(store), 'utf8'); renameSync(temporaryPath, stagedPath); }
  finally { rmSync(temporaryDirectory, { recursive: true, force: true }); }
};

const referencedAssetPaths = (records) => {
  ...
  for (const record of records) {
    ...
    if (record.content?.image?.src) referenced.add(record.content.image.src);
    if (record.content?.gallery) for (const media of record.content.gallery) {
      if (media?.src) referenced.add(media.src);
      if (media?.poster) referenced.add(media.poster);
    }
    for (const section of record.content?.sections ?? []) collect(section.nodes);
  }
  return referenced;
};

const pruneStagedAssets = (assets, records, baseline) => {
  const effectiveRecords = baseline
    ? mergeCaseStudyManifest(baseline, { records, claims: {}, assets }).records
    : records;
  const referenced = referencedAssetPaths(effectiveRecords);
  return Object.fromEntries(Object.entries(assets ?? {}).filter(([publicPath]) => referenced.has(publicPath)));
};
...
export async function stageReviewedCaseStudyCandidate({ candidatePath, stagedPath, metadata }) {
  ...
  const existing = readStagedCaseStudyPublication(stagedPath);
  ...
  const combinedAssets = { ...(existing.assets ?? {}), ...assetEntries };
  const proposedStagedStore = { records, claims, assets: pruneStagedAssets(combinedAssets, records, baseline) };
  ...
  try {
    writeStagedCaseStudyPublication(stagedPath, { records, claims, assets: proposedStagedStore.assets });
  } catch (error) {
    for (const file of newlyInstalled) rmSync(file, { force: true });
    throw error;
  }
  return {
    candidateSha256: actualDigest,
    records,
    affected: stagedRecords.map((record) => ({ id: record.id, slug: record.slug })),
    stagedPath,
  };
}

/**
 * Explicitly withdraw one identity from the existing staged publication.
 * Omitted records are deliberately untouched; a draft record is the tombstone
 * that masks a same-id baseline record during manifest merging.
 */
export async function withdrawStagedCaseStudy({ id, stagedPath }) {
  if (typeof id !== 'string' || !slugPattern.test(id)) throw new Error('Withdrawal requires a safe case-study --id');
  const existing = readStagedCaseStudyPublication(stagedPath);
  const manifestPath = path.join(path.dirname(stagedPath), 'case-study-manifest.js');
  const manifestModule = await import(`${pathToFileURL(manifestPath).href}?cacheBust=${Date.now()}`);
  const baseline = manifestModule.caseStudyPublicationBaseline ?? manifestModule.caseStudyPublicationManifest;
  if (!baseline) throw new Error(`Manifest does not export a stable baseline: ${manifestPath}`);
  const currentManifest = mergeCaseStudyManifest(baseline, existing);
  const current = currentManifest.records.find((record) => record.id === id);
  if (!current) throw new Error(`Cannot withdraw unknown case-study id ${id}`);
  const tombstone = { id, slug: current.slug, status: 'draft' };
  const records = existing.records.some((record) => record.id === id)
    ? existing.records.map((record) => record.id === id ? tombstone : record)
    : [...existing.records, tombstone];
  const claims = Object.fromEntries(Object.entries(existing.claims).filter(([claimId, claim]) => claim?.recordId !== id && !claimId.startsWith(`${id}.`)));
  const stagedStore = { records, claims, assets: pruneStagedAssets(existing.assets, records, baseline) };
  const proposedManifest = mergeCaseStudyManifest(baseline, stagedStore);
  const { compileCaseStudyPublication } = await import('./compile-case-studies.js');
  const repositoryRoot = path.resolve(path.dirname(stagedPath), '..');
  compileCaseStudyPublication({ manifest: proposedManifest, root: repositoryRoot });
  const changed = JSON.stringify(existing) !== JSON.stringify(stagedStore);
  if (changed) writeStagedCaseStudyPublication(stagedPath, stagedStore);
  return { id, slug: current.slug, changed, stagedPath, records };
}
