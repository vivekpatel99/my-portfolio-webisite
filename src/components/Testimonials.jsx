...
const TestimonialCard = ({ projectTitle, clientName, content, link }) => (
  ...
  >
    <div className="flex flex-col h-full bg-[#1E1E2A] p-6 rounded-2xl border border-white/10 transition-all duration-300 hover:border-accent-purple/50 hover:shadow-lg hover:shadow-accent-purple/10">
...
const MarqueeRow = ({ items, direction = 'left', duration = 45 }) => (
  <div className="testimonial-marquee-row">
    ...
    >
      {[...items, ...items].map((testimonial, index) => (
        <div key={`${testimonial.id}-${index}`} className="testimonial-marquee-item">
      ...
      <div className="container mx-auto px-6">
        ...
        {shouldReduceMotion ? (
          ...
          <div className="testimonial-marquee">
            <MarqueeRow items={firstRow} direction="left" duration={42} />
            <MarqueeRow items={secondRow} direction="right" duration={38} />
          </div>
        )}
      </div>
    </section>
